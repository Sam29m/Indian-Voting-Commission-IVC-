...
app.use('/api/auth', require('./routes/securityRoutes'));
...
app.use('/api/mitra', require('./routes/mitraRoutes'));

// Health check
app.get('/api/health', (req, res) => {
  res.json({ status: 'ok', service: 'Indian Voting Commission API', version: '2.0.0' });
});
...
async function autoSeed() {
  ...
  await Election.create({
    ...
    type: 'general', constituency: 'National', status: 'scheduled',
    candidates: [
...
connectDB().then(async () => {
  await autoSeed();
  app.listen(PORT, () => {
    console.log(`\n🗳️  Indian Voting Commission API v2.0`);
