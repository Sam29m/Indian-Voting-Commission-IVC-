...
exports.login = async (req, res) => {
  try {
    ...
    if (!user) return res.status(401).json({ message: 'Invalid email (User not found)' });

    const isMatch = await user.comparePassword(password);
    if (!isMatch) return res.status(401).json({ message: 'Invalid password (Password mismatch)' });
